...
import { geocodeVoterRecords } from "../lib/geocoding";
...
export function DataUpload() {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    if (kind === "election-history" && file.name.toLowerCase().endsWith(".csv")) {
      ...
      setStatus(`Imported ${electionRecords.length.toLocaleString()} election records. Geocoding unique addresses with Census batch first.`);
      const geocoding = await geocodeVoterRecords(deduped.records, true);
      setVoterState({ analytics, voterPoints: geocoding.points, geocodeReviewItems: geocoding.reviewItems });
      setStatus(
        `Imported ${electionRecords.length.toLocaleString()} records. Stored ${geocoding.points.length.toLocaleString()} geocoded household markers; ${geocoding.reviewItems.length.toLocaleString()} addresses need review.`,
      );
      return;
    }
